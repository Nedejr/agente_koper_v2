...
def test_video_in_response():
    ...
    try:
        ...
        print(f"   {'✅' if has_youtube_embed else '❌'} Contém [YOUTUBE_EMBED:] - {has_youtube_embed}")
        print(f"   {'✅' if has_video_emoji else '❌'} Contém emoji 🎬 - {has_video_emoji}")
        print(f"   {'✅' if has_video_tag else '❌'} Contém tag [video:] - {has_video_tag}")
        print()
        ...
        for i, doc in enumerate(source_docs[:3], 1):
            ...
            if video_timestamps:
                ...
                print(f"         Primeiro: {first_ts.get('start', 'N/A')} → {first_ts.get('end', 'N/A')}")
            print()
